# configs/vit_config.py

import os
from pathlib import Path

class ViTConfig:
    """Конфигурация для обучения Vision Transformer с GPU оптимизациями"""
    ...
    # Параметры модели
    MODEL_NAME = 'vit_base_patch16_224_gpu'
    NUM_CLASSES = 43
    INPUT_SIZE = 224
    PATCH_SIZE = 16
    
    # Параметры обучения
    BATCH_SIZE = 64  # Уменьшен для ViT (требует больше памяти)
    EPOCHS = 15
    LEARNING_RATE = 0.001
    WEIGHT_DECAY = 0.05
    
    # GPU оптимизации - ОТКЛЮЧАЕМ USE_HALF_PRECISION
    USE_HALF_PRECISION = False  # Отключаем, используем только AMP
